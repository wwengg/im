...
func InitGinHttpGate() *http.GinEngine {
	...
	publicGroup.Use(middleware.BaseHandler(), middleware.RPCXHandler(), middleware.V1Handler(), middleware.JwtHandler(), middleware.CasbinHandler())
	{
		router.InitSRPCRouter(publicGroup)
	}
	privateGroup.Use(middleware.BaseHandler(), middleware.RPCXHandler(), middleware.V2Handler(), middleware.JwtHandler(), middleware.CasbinHandler())
	{
